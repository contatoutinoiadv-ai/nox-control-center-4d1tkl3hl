...
> **Status:** Concluído (Fase 2 - Versão 0.0.54)  
> **Data:** Março de 2026  
...
| :-- | :--------------------------------- | :---------------------------------------- | :-------------------------------- | :------------------------------------ | :------------------- | :---------------------------------- | :---------- | :-------------------------------- | :--------------------------------------- | :----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1   | `nox_control_center_clients_v1`    | `src/services/dataStore.ts`               | `NoxClient[]` (JSON)              | Clientes & Atendimentos               | `init()` (linha 171) | `saveClients()` (linha 872)         | **ALTA**    | Sim (`clients`)                   | **CONCLUÍDO (v0.0.54)**                  | Migrado para `clients` no PocketBase como Source of Truth oficial. Leitura prioritária via PB, mutação remota, cache em memória RAM e contingência offline local. Realtime SSE habilitado. Remapeamento via `legacy_id`. |
| 2   | `nox_sentinela_agenda_v1`          | `src/services/dataStore.ts`               | `AgendaEvent[]` (JSON)            | Central de Prazos / Compromissos      | `init()` (linha 156) | `saveAgenda()` (linha 2521)         | **ALTA**    | Sim (`sentinela_agenda`)          | **CONCLUÍDO (v0.0.54)**                  | Migrado para `sentinela_agenda` no PocketBase como Source of Truth oficial. Leitura e escrita no PB com realtime SSE ativo. Remapeamento de `client_id` canônico e preservação de `legacy_id`.                           |
| 3   | `nox_control_center_production_v1` | `src/services/dataStore.ts`               | `ProductionItem[]` (JSON)         | Produção de Peças                     | `init()` (linha 178) | `saveProductionItems()` (linha 881) | **ALTA**    | Sim (`production_items`)          | **CONCLUÍDO (v0.0.54)**                  | Migrado para `production_items` no PocketBase como Source of Truth oficial. Relacionamento estrito com `client_id` (15 caracteres), persistência direta no PB, realtime SSE e `legacy_id`.                               |
| 4   | `nox_sentinela_tasks_v1`           | `src/services/dataStore.ts`               | `SentinelaTask[]` (JSON)          | Central de Prazos / Sentinela         | `init()` (linha 153) | `saveTasks()` (linha 2479)          | **ALTA**    | Sim (`sentinela_tasks`)           | **CONCLUÍDO (v0.0.54)**                  | Migrado para `sentinela_tasks` no PocketBase como Source of Truth oficial. Sincronização direta com o backend, subscrição SSE em tempo real e rastreabilidade por `legacy_id`.                                           |
...
| :----------------- | :--------------------------------------------------- | :------------------------------------------------------ | :------------------------------------------------------------ | :------------------------------------------------------------------------------------------ | :----------------------------------------------------------------- | :---------------------------------------------------------------------------- |
| **CLIENTES**       | Dual (`localStorage` + sync tardio PB)               | **PocketBase (`clients`) — EFETIVADO v0.0.54**          | `clients`                                                     | `pb.collection('clients').getFullList({ sort: '-created' })`                                | `pb.collection('clients').create()` / `.update()`                  | Se offline, exibe aviso de sincronização pendente e retém em memória RAM.     |
...
| **PRODUÇÃO**       | Dual (`localStorage` + background sync PB)           | **PocketBase (`production_items`) — EFETIVADO v0.0.54** | `production_items`                                            | `pb.collection('production_items').getFullList({ sort: '-created' })`                       | `pb.collection('production_items').create()` / `.update()`         | Atualização reativa de interface com salvamento assíncrono garantido no PB.   |
| **COMPROMISSOS**   | Dual (`localStorage` + background sync PB)           | **PocketBase (`sentinela_agenda`) — EFETIVADO v0.0.54** | `sentinela_agenda`                                            | `pb.collection('sentinela_agenda').getFullList({ sort: '-start_date' })`                    | `pb.collection('sentinela_agenda').create()` / `.update()`         | Sincronizado com status de preparação de audiência (`preparacao_habilitada`). |
...
| **AUDITORIA**      | Dual (`localStorage` + PB pontual)                   | **PocketBase (`audit_logs`) — EFETIVADO v0.0.54**       | `audit_logs`                                                  | `pb.collection('audit_logs').getFullList({ sort: '-created' })`                             | `pb.collection('audit_logs').create()`                             | Append-only imutável. Regras de API impedem update e delete.                  |
...
| **SENTINELA**      | Dual (`localStorage` + ingestão DJEN)                | **PocketBase (`sentinela_tasks` EFETIVADO v0.0.54)**    | `sentinela_communications`, `sentinela_tasks`                 | Ingestão DJEN direta no frontend gravando em `sentinela_communications`                     | `pb.collection('sentinela_communications')`                        | Snapshot de custódia com hash SHA-256 e timeline gravados no banco.           |
...
## 6. STATUS DE EFETIVAÇÃO DA FASE 2 (VERSÃO 0.0.54 - MARÇO 2026)

Conforme implementado e validado nos lotes 2B, 2C e 2D:

1. **Domínios Oficialmente Migrados para PocketBase Source of Truth:**
   - **Clientes (`clients`):** Concluído. Leitura prioritária via PocketBase `getFullList`, mutações propagadas diretamente para a collection oficial, subscrição SSE ativa para sincronização em tempo real entre operadores, persistência de `legacy_id` para rastreabilidade de identificadores legados.
   - **Compromissos / Audiências (`sentinela_agenda`):** Concluído. PocketBase é autoritativo, suporte a SSE, resolução canônica de `client_id` (15 caracteres) a partir de CPF/código, preservação de `preparacao_habilitada` e `legacy_id`.
   - **Produção Jurídica (`production_items`):** Concluído. Validação estrita de `client_id` canônico, persistência de esteira, triagem de 5 camadas e stress-test no PocketBase, SSE ativo e `legacy_id`.
   - **Tarefas (`sentinela_tasks`):** Concluído. PocketBase autoritativo, subscrição SSE, sincronização bidirecional resiliente e `legacy_id`.
   - **Auditoria (`audit_logs`):** Concluído. Migração 0018/0019 garantindo categorias `migracao` e `seguranca`. Logs append-only imutáveis gravados no PocketBase.

2. **Chaves Locais Preservadas e Justificativa:**
   - `nox_control_center_settings_v1`: Mantido localmente (preferências de UI de cada operador: auto-refresh, motion reduzido, refresh interval).
   - `nox_lawyer_profile_v1` / dados de perfil no store: Mantido localmente para customização de visualização do operador titular sem poluir configurações corporativas globais.
   - `nox_migrated_v2_*`: Flags de idempotência criadas pelo `legacyStorageAdapter` para impedir reimportações redundantes e loops de sincronização.
   - Cache de contingência offline (`localStorage`): Mantido estritamente como fallback resiliente e temporário para cenários em que o operador perde conexão com o PocketBase.
